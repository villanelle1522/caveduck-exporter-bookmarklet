(() => {
  ...
  const stateFrom = data => {
    ...
    const player = characters.find(item => /^(protagonist|user|player)$/i.test(String(item.role || ''))) || {};
    ...
    return { gameTime, environmentTime: env.time || '', storyTime, place: env.place || '', weather: env.weather || '', scene: [gameTime, env.place, env.weather].filter(Boolean).join(' · '), playerName: player.name || '', name: character.name || '', emotion: character.emotion || '', relation: character.relation && typeof character.relation === 'object' ? character.relation : {} };
  };
  const normalize = chat => {
    ...
    return [
      query && { id: `${chat.id}-q`, turnId: chat.id, order: 0, user: true, name: state.playerName || '玩家', text: query, createdAt: chat.created_at || '', time, state, image, deep },
...
<body>
  ...
  function renderDashboard(list, records, chapterData, media) {
    ...
    const rawRecords = records.map(record => ({
      ...
      environment_state: { time: record.state.environmentTime, story_time: record.state.storyTime, place: record.state.place, weather: record.state.weather, player: record.state.playerName, character: record.state.name, emotion: record.state.emotion, relation: record.state.relation },
    ...
    return `<div class="dashboard">
      <header class="dashboard-heading"><h1 id="audit-title">對話資料儀表板</h1></header>
